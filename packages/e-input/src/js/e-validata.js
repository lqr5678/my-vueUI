...
export function custom(rule, value, callback,erule) {
  ...
  if(typeof erule === 'string'){
    setrule.hintErr = erule
  }else{
    setrule.hintErr = erule.hintErr,
    setrule.uetest = erule.uetest
  }
  if (value == '' || value == undefined || value == null) {
    callback(new Error(`${setrule.hintErr}不能为空`));
  } else {
    if(!setrule.uetest) return callback();
    if ((!setrule.uetest.test(value)) && value != '') {
      callback(new Error(`${setrule.hintErr}填写错误，请重新填写`));
    } else {
      callback();
    }
  }
